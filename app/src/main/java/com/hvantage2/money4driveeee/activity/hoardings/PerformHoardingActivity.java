...
public class PerformHoardingActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setRecycler_view();
        tvProjectTittle.setText(AppPreference.getSelectedHoardingName(PerformHoardingActivity.this));
    }
    ...
    private void setRecycler_view() {
        RecyclerView.LayoutManager layoutManager = new LinearLayoutManager(PerformHoardingActivity.this);
        recycler_view.setLayoutManager(layoutManager);
        adapter = new ActivityAdapter(PerformHoardingActivity.this, activityList);
        recycler_view.setAdapter(adapter);
        recycler_view.addOnItemTouchListener(new RecyclerItemClickListener(PerformHoardingActivity.this, recycler_view, new RecyclerItemClickListener.OnItemClickListener() {
            @Override
            public void onItemClick(View view, int position) {

                ShopActivity model = activityList.get(position);
                if (activityList.get(position).getActivity_status() == 1) {
                    Intent intent = new Intent(PerformHoardingActivity.this, SingleActivityDetail.class);
                    intent.setAction("hoarding");
                    intent.putExtra("media_option_id", model.getActivity_id());
                    intent.putExtra("media_option_name", model.getActivity_name());
                    startActivity(intent);
                } else {
                    Intent intent = new Intent(PerformHoardingActivity.this, UploadPhotosActivity.class);
                    intent.setAction("hoarding");
                    intent.putExtra("media_option_id", model.getActivity_id());
                    intent.putExtra("media_option_name", model.getActivity_name());
                    startActivity(intent);
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.imagViewEdit) {
            Intent intent = new Intent(PerformHoardingActivity.this, HoardingDetailActivity.class);
    ...
    class GetActivityTask extends AsyncTask<Void, String, Void> {
        ...
        protected Void doInBackground(Void... voids) {
            ...
            jsonObject.addProperty("user_id", AppPreference.getUserId(PerformHoardingActivity.this));
            jsonObject.addProperty("project_id", AppPreference.getSelectedProjectId(PerformHoardingActivity.this));
            jsonObject.addProperty("hoarding_id", AppPreference.getSelectedHoardingId(PerformHoardingActivity.this));
        ...
        protected void onProgressUpdate(String... values) {
            ...
            if (status.equalsIgnoreCase("400")) {
                Toast.makeText(PerformHoardingActivity.this, msg, Toast.LENGTH_SHORT).show();
            }
        }
    }
