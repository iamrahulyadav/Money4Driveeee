...
public class PerformEMediaActivity extends AppCompatActivity implements View.OnClickListener {
    private static final String TAG = "PerformEMediaA";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setRecycler_view();
        tvProjectTittle.setText(AppPreference.getSelectedEMediaName(PerformEMediaActivity.this));
    }
    ...
    private void setRecycler_view() {
        RecyclerView.LayoutManager layoutManager = new LinearLayoutManager(PerformEMediaActivity.this);
        recycler_view.setLayoutManager(layoutManager);
        adapter = new ActivityAdapter(PerformEMediaActivity.this, activityList);
        recycler_view.setAdapter(adapter);
        recycler_view.addOnItemTouchListener(new RecyclerItemClickListener(PerformEMediaActivity.this, recycler_view, new RecyclerItemClickListener.OnItemClickListener() {
            @Override
            public void onItemClick(View view, int position) {

                ShopActivity model = activityList.get(position);
                if (activityList.get(position).getActivity_status() == 1) {
                    Intent intent = new Intent(PerformEMediaActivity.this, SingleActivityDetail.class);
                    intent.setAction("emedia");
                    intent.putExtra("media_option_id", model.getActivity_id());
                    intent.putExtra("media_option_name", model.getActivity_name());
                    startActivity(intent);
                } else {
                    Intent intent = new Intent(PerformEMediaActivity.this, UploadPhotosActivity.class);
                    intent.setAction("emedia");
                    intent.putExtra("media_option_id", model.getActivity_id());
                    intent.putExtra("media_option_name", model.getActivity_name());
                    startActivity(intent);
    ...
    private void init() {
        ...
        ((CustomTextView) findViewById(R.id.tvTitle)).setText("Radio/Cinema");
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.imagViewEdit) {
            Intent intent = new Intent(PerformEMediaActivity.this, EMediaDetailActivity.class);
    ...
    class GetActivityTask extends AsyncTask<Void, String, Void> {
        ...
        protected Void doInBackground(Void... voids) {
            ...
            jsonObject.addProperty("method", AppConstants.FEILDEXECUTATIVE.PROJECTACTIVITYLIST);
            jsonObject.addProperty("user_id", AppPreference.getUserId(PerformEMediaActivity.this));
            jsonObject.addProperty("project_id", AppPreference.getSelectedProjectId(PerformEMediaActivity.this));
            jsonObject.addProperty("emedia_id", AppPreference.getSelectedEMediaId(PerformEMediaActivity.this));
        ...
        protected void onProgressUpdate(String... values) {
            ...
            if (status.equalsIgnoreCase("400")) {
                Toast.makeText(PerformEMediaActivity.this, msg, Toast.LENGTH_SHORT).show();
            }
        }
    }
}
